class CreateTipoFrecuencias < ActiveRecord::Migration[6.0]
  def change
    create_table :tipo_frecuencias, id: false, comment: "Catálogo de tipos de frecuencia" do |t|
      ...
      t.string :descripcion, limit: 200, null: true, comment: "Descripción general del tipo de frecuencia"
    ...
    execute <<-SQL
      alter table tipo_frecuencias add
        constraint pk_tipo_frecuencia
        primary key (id)
    SQL

    # Agregar el índice único con el nombre personalizado
    add_index :tipo_frecuencias, [:empresa_id, :nombre], name: "idx_tipoFrecuencia", unique: true

    # Agregar el constraint CHECK sin el punto y coma al final
    execute <<-SQL
      alter table tipo_frecuencias add
        constraint ck_estado_tipoFrecuencia
        check (estado in ('A', 'I'))
    SQL

    # Agregar el valores default de registro/actualizacion de usuario en base de datos
    execute <<-SQL
      ALTER TABLE tipo_frecuencias
